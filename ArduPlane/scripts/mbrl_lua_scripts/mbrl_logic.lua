...
assert(param:add_param(PARAM_TABLE_KEY, 7,  'STATUS', 0), 'could not add param10')
DO_JUMP = 177
k_throttle = 70
...
local AERO_STATUS = bind_param("AEROM_STATUS")
...
function do_loop(arg1, arg2)
  ...
  if math.abs(pitch_deg) > 85 and  math.abs(pitch_deg) < 95 then
   ...
   return throttle, roll_rate, pitch_rate, 0
end
...
function send_state()
  ...
  local velocity_b = ahrs:earth_to_body(velocity)

  -- gcs:send_text(0, string.format(initial_height))

  gcs:send_named_float(string.format("x"), position:x() - initial_position:x())
  gcs:send_named_float(string.format("y"), position:y() - initial_position:y())
  gcs:send_named_float(string.format("z"), position:z() - initial_position:z())
  gcs:send_named_float(string.format("roll"), roll)
  gcs:send_named_float(string.format("pitch"), pitch)
  gcs:send_named_float(string.format("yaw"), yaw)
  gcs:send_named_float(string.format("u"), velocity_b:x())
  gcs:send_named_float(string.format("v"), velocity_b:y())
  gcs:send_named_float(string.format("w"), velocity_b:z())
  gcs:send_named_float(string.format("p"), rates:x())
  gcs:send_named_float(string.format("q"), rates:y())
  gcs:send_named_float(string.format("r"), rates:z())
  -- if state == 2 then
  now = millis():tofloat() * 0.001
  gcs:send_named_float(string.format("time"), now - do_rl_ms)
  -- else
  --   gcs:send_named_float(string.format("time"), 0)
  -- end
end

function get_acro_actions(arg1, arg2)
   throttle, roll_rate, pitch_rate, yaw_rate = do_loop(arg1, arg2)
   return throttle, roll_rate, pitch_rate, yaw_rate
end
...
function do_rl()
  ...
  local status = AERO_STATUS:get()
  -- gcs:send_text(0, string.format("Status: %d", status))
  if status == 0 then
    gcs:send_text(0, string.format("BUSY"))
    vehicle:nav_script_time_done(last_id)
    return
  end    
    if not running then
        running = true
        gcs:send_text(0, string.format("RL READY"))
        AERO_STATUS:set(2)
    end
    
    -- gcs:send_text(0, string.format(vehicle:get_target_rate_rpy()[1]))
    if status == 3 then
      gcs:send_text(0, string.format("RESET"))
      initial_yaw = ahrs:get_yaw()
      -- initial_height = ahrs:position():z()
      initial_position =  ahrs:get_relative_position_NED_origin()
      initial_vel = ahrs:get_velocity_NED()
      initial_vel = ahrs:earth_to_body(initial_vel)
      now = millis():tofloat()*0.001
      do_rl_ms = now
      AERO_STATUS:set(4)
    elseif status == 4 then
      gcs:send_text(0, string.format("SENDING STATE"))
      if acro_flag == true then
         throttle_target, roll_rate, pitch_rate, yaw_rate = get_acro_actions(arg1, arg2)
      else
         throttle_target = vehicle:get_target_throttle()
         roll_rate, pitch_rate, yaw_rate = vehicle:get_target_rate_rpy()
      end
      send_state(initial_yaw_deg, initial_height, throttle_target, roll_rate, pitch_rate, yaw_rate)
      gcs:send_text(0, string.format("%f,%f,%f,%f", rpy_rate:x(), rpy_rate:y(), rpy_rate:z(), throttle_target))
      vehicle:set_target_throttle_rate_rpy(throttle_target, rpy_rate:x(),0, rpy_rate:z())
      -- SRV_Channels:set_output_pwm_chan_timeout(elevator_channel, math.floor(rpy_rate:y()), 1000)
    elseif status == 5 then
      gcs:send_text(0, string.format("RL TERMINAL"))
      vehicle:nav_script_time_done(last_id)
      AERO_STATUS:set(1)
      return
    end
...
function update()
  ...
  if id then
      ...
      if count ==0 then
      --   gcs:send_text(0, "Writing to param")
      --   state = 1
        count = count + 1
      end
      do_rl()
